...
시계열 분류 모델에서도 비슷한 문제가 발생한다. 예를 들어 특정 임상 이벤트인 P, P+1, P+2, P+3 구간을 예측하는 모델을 평가한다고 하자. 연구 목적이 해당 critical window에서의 분류 성능이라면 confusion matrix의 denominator도 그 구간에 맞추어 해석해야 한다. 그런데 전체 시계열 구간을 denominator로 사용하면, 실제 관심 구간에서의 예측 성능이 희석되거나 왜곡될 수 있다. 이 경우 문제는 단순히 코드 구현의 오류가 아니라 평가 설계의 문제다.
